...
import { ArrowLeft, FileText, Save, Scale, Trash2 } from 'lucide-react';
...
export default function DocumentTypeEdit() {
    const { documentType, sensitivityOptions, canDelete } = usePage()
        .props as unknown as {
        documentType: any;
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        sensitivity_level: documentType?.sensitivity_level || 'internal',
        retention_policy: documentType?.retention_policy || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        setIsSubmitting(true);
        put(`${basePath}/${documentType.id}`, {
            preserveScroll: true,
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: `Edit: ${documentType?.code}`, href: '#' },
            ]}
        >
            <Head title={`Edit Document Type - ${documentType?.name}`} />
            ...
            <div className="flex min-h-[calc(100vh-64px)] w-full flex-col bg-muted/10">
                ...
                <div className="w-full flex-1 p-4 md:p-8 lg:p-12">
                    ...
                    <div className="mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-start">
                        <div>
                            ...
                            <p className="mt-2 text-base text-muted-foreground">
                                Update the classification and retention rules
                                for this document category.
                            </p>
                        </div>

                        {/* Additional Delete Action for Edit View */}
                        <AlertDialog
                            ...
                            <AlertDialogContent>
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                        ...
                                        delete the document type classification{' '}
                    ...
                    <form id="edit-doc-type-form" onSubmit={handleSubmit}>
                                ...
                                <Card className="border-dashed border-border bg-muted/10 shadow-sm">
                                    <CardContent className="space-y-4 p-6">
                                        ...
                                        <p className="text-xs font-medium text-muted-foreground">
                                            Modifying the retention policy will
                                            flag existing files for review
                                            during the next compliance cycle.
                                        </p>
                                    </CardContent>
                                </Card>
                            </div>
                ...
                <div className="sticky bottom-0 z-40 flex w-full items-center justify-between border-t bg-background p-4 px-6 shadow-[0_-4px_10px_-1px_rgba(0,0,0,0.05)] md:px-12">
                    ...
                    <div className="flex items-center gap-3">
                        <Button
                            ...
                            onClick={() =>
                                router.visit(`${basePath}/${documentType.id}`)
                            }
                        >
                            Cancel
                        </Button>
