...
export default function PositionEdit() {
    const { position, departments } = usePage().props as unknown as {
        position: any;
        departments: OrgUnit[];
    };
    ...
    const { data, setData, put, processing, errors } = useForm({
        name: position?.name || '',
        code: position?.code || '',
        org_unit_id: position?.org_unit_id?.toString() || '',
                    ...
                    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="max-w-2xl">
                            ...
                            <p className="mt-2 text-base text-muted-foreground">
                                Modify organizational role parameters and
                                hierarchy details.
                            </p>
                        </div>
                        <div className="flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1.5 text-sm font-semibold text-primary">
                ...
                <div className="sticky bottom-0 z-40 flex flex-col items-center justify-between gap-4 border-t bg-background p-4 px-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] sm:flex-row md:px-8">
                    ...
                    <div className="flex w-full items-center gap-3 sm:w-auto">
                        <Button
                            ...
                            onClick={() =>
                                router.visit(`${API}/positions/${position.id}`)
                            }
                        >
                            Discard Changes
                        </Button>
