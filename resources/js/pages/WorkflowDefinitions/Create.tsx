...
export default function WorkflowDefinitionCreate() {
    const { data, setData, post, processing, errors } = useForm({
        name: '',
        request_type: '',
        sla_hours: '48',
        status: 'Draft',
        steps_json: [] as WorkflowStep[],
    });

    const [isSubmitting, setIsSubmitting] = useState(false);

    // Initial mock state to match the screenshot
    useState(() => {
        if (data.steps_json.length === 0) {
            setData('steps_json', [
                {
                    id: crypto.randomUUID(),
                    type: 'Direct Manager',
                    config: 'Reports to Requestor',
                },
                {
                    id: crypto.randomUUID(),
                    type: 'Specific Role',
                    config: 'HR Administrator',
                },
            ]);
        }
    });
    ...
    const handleRemoveStep = (id: string) => {
        setData(
            'steps_json',
            data.steps_json.filter((step) => step.id !== id),
        );
    };
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: 'Configure Routing', href: '#' },
            ]}
        >
            <Head title="Configure Workflow Routing" />
            ...
            <div className="flex min-h-[calc(100vh-64px)] w-full flex-col bg-muted/10">
                                ...
                                <Card className="border-border bg-background shadow-sm">
                                    ...
                                    <CardContent className="space-y-4 p-6">
                                        {data.steps_json.map((step, index) => (
                                            <div
                                                key={step.id}
                                                className="flex items-stretch gap-3 rounded-xl border border-border/50 bg-muted/20 p-4"
                                            >
                                        ...
                                        <Button
                                            type="button"
                                            variant="outline"
                                            className="mt-4 h-14 w-full border-2 border-dashed font-semibold text-muted-foreground hover:text-foreground"
                ...
                <div className="sticky bottom-0 z-40 flex flex-col items-center justify-between gap-4 border-t bg-background p-4 px-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] sm:flex-row md:px-8">
                    <div className="text-sm font-medium text-muted-foreground">
                        {/* Mock unsaved changes text to match screenshot */}
                    ...
                    <div className="flex w-full items-center gap-3 sm:w-auto">
                        <Button
                            ...
                            onClick={() =>
                                router.visit(`${API}/workflow_definitions`)
                            }
                        >
                            Cancel
                        </Button>
