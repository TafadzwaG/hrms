...
export default function DocumentEdit() {
    const { document, employees, documentTypes, accessPolicyOptions } =
    ...
    const { data, setData, put, processing, errors } = useForm({
        owner_employee_id: document?.owner_employee_id?.toString() || '',
        document_type_id: document?.document_type_id?.toString() || '',
        title: document?.title || '',
        file_uri: document?.file_uri || '',
        issue_date: document?.issue_date
            ? document.issue_date.split('T')[0]
            : '',
        expiry_date: document?.expiry_date
            ? document.expiry_date.split('T')[0]
            : '',
        access_policy: document?.access_policy || 'internal',
        metadata_json: document?.metadata_pretty || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        setIsSubmitting(true);
        put(`${basePath}/${document.id}`, {
            preserveScroll: true,
    ...
    return (
        <AppLayout
            breadcrumbs={[
                ...
                { title: `Edit: ${document?.title || 'Document'}`, href: '#' },
            ]}
        >
                ...
                <div className="w-full flex-1 p-4 md:p-8 lg:p-12">
                    ...
                    <div className="mb-8 space-y-4">
                        <Button
                            ...
                            onClick={() =>
                                router.visit(`${basePath}/${document.id}`)
                            }
                        >
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-base text-muted-foreground">
                                Update the details and metadata for this
                                registry record.
                            </p>
                        </div>
                    </div>
